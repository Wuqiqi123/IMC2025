...
def get_reconstructed_scene(glomap_bin, outdir, gradio_delete_cache, model, retrieval_model, device, silent, image_size,
    ...
    submission_file = 'result/submission_test.csv'
    with open(submission_file, 'w') as f:
        ...
        for idx, (colmap_imgid, colmap_image) in enumerate(ouput_recon.images.items()):
            ...
            rotation = array_to_str(colmap_image.cam_from_world.rotation.matrix().flatten())
            translation = array_to_str(colmap_image.cam_from_world.translation)

            f.write(f'{colmap_image.name},{rotation},{translation}\n')
            # colmap_world_to_cam[colmap_imgid] = colmap_image.cam_from_world.rotation.matrix()
...
from dust3r.dust3r.demo import set_print_with_timestamp
import pycolmap
import os
import torch
import tempfile
...
if __name__ == '__main__':
    ...
    model = AsymmetricMASt3R.from_pretrained("ckpts/MASt3R_ViTLarge_BaseDecoder_512_catmlpdpt_metric.pth").to(args.device)
    chkpt_tag = hash_md5("ckpts/MASt3R_ViTLarge_BaseDecoder_512_catmlpdpt_metric.pth")
